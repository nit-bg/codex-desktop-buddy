...
static const size_t RX_CAP = 8192;   // enlarged so folder-push bursts can't overflow the ring
...
void bleInit(const char* deviceName) {
  ...
  // Open GATT: no pairing, no encryption. The desktop bridge talks to us over
  // bleak (Python), and BLE Secure-Connections + MITM passkey pairing is
  // unreliable on Windows; an unencrypted NUS link connects with zero pairing
  // friction. Trade-off: the link is sniffable in radio range — fine for a
  // local desk gadget. (See the encrypted/Just-Works variants in git history.)
