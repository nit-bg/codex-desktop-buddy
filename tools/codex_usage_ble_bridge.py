...
def codex_cli_path(args: argparse.Namespace) -> Path | None:
    ...
    if found:
        return Path(found)
    # Known install locations per OS.
    candidates = [DEFAULT_CODEX_APP_CLI]  # macOS app bundle
    local = os.environ.get("LOCALAPPDATA")
    if local:  # Windows: %LOCALAPPDATA%\OpenAI\Codex\bin\codex.exe
        candidates.append(Path(local) / "OpenAI" / "Codex" / "bin" / "codex.exe")
...
def read_app_server_usage(args: argparse.Namespace, activity: UsageSnapshot | None) -> UsageSnapshot | None:
    ...
    try:
        ...
        # Read replies on a background thread — select() can't poll pipes on
        # Windows (sockets only). Queue lines and drain with a timeout so the
        # deadline / process-exit checks still run. Cross-platform.
        ...
        def _reader(out):
            try:
                ...
                line_q.put(None)   # stream closed / EOF

        _threading.Thread(target=_reader, args=(proc.stdout,), daemon=True).start()
